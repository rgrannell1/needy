...
report <- list(
	...
	missing_rules = function (pcall) {
		stopf (
			"%s: the parameter 'rules' was missing but is required\n", 
			pcall)
	},
	...
	rules_not_list = function (pcall, rules) {
		stopf(
			"%s: the parameter 'rules' must be a list\n
			actual class was %s
			", 
			pcall, paste0(class(rules), collapse = ", "))
		)
	},
